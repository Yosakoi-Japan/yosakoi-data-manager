...
## Decision 4: 過去開催イベントも公開 CSV に保持する

- **Decision**: `Approved` の過去開催イベントを削除せず、`end_date` は監視件数と
  フロントエンド表示用の判定材料として使う。
- **Rationale**: 受賞履歴を `event_id` で参照可能にし、アーカイブ価値を維持するため。
- **Alternatives considered**:
  - `start_date` で除外判定: 開催中イベントを誤除外する。
  - 期間文字列をパースする: 現在の実データ構造では不要。
  - 過去イベントを同期除外: 受賞履歴の参照先が失われる。
...
## Decision 10: 受賞チームは別シート・別 CSV で管理する

- **Decision**: 既存 `events` シートは変更せず、`award_winners` シートを追加する。
  公開側も固定8列の `award_winners.csv` とし、`event_id` でイベントへ関連付ける。
- **Rationale**: イベント1件に共同受賞を含む複数チームを持て、イベント列を肥大化させない。
  代表動画は公開必須とし、運営公式・チーム公式・一般投稿の優先度を明示できる。
- **Alternatives considered**:
  - events に受賞列を追加: 複数受賞者を自然に表現できない。
  - 賞と受賞者を複数シートへ分割: 現在の最上位賞のみというスコープには過剰。
